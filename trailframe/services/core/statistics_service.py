...
class StatisticsService(Service):
    ...
    def record_run(cls, stats: list[dict]) -> None:
        def _record(session):
            async def _run(s):
                for stat in stats:
                    s.add(ScannerStat(scanner=stat["scanner"], count=stat["count"], total_ms=stat["total_ms"]))

                await s.commit()

            return _run(session)

        DatabaseService.execute_detached(_record)

    @classmethod
    async def get_scanner_summary(cls) -> list[ScannerStatSummary]:
        async def _summary(session) -> list[ScannerStatSummary]:
            ...
            for scanner, items, total_ms in rows:
                summary.append(
                    ScannerStatSummary(
                        name=scanner, items=items, value=items / (total_ms / 1000) if total_ms > 0 else 0.0
                    )
                )

            return summary

        return await DatabaseService.execute(_summary)
